...
def test_entrada_producto_existente_incrementa_stock_y_redirige(create_jwt):
    data_insert = {"valor_id": 1, "cantidad_entrada": 20}

    response = create_jwt.post("/registrar_entrada_existente", data=data_insert)

    pool,cursor = instancia_conexion.iniciar_conexion()
    producto = instancia_conexion.uno(cursor, "SELECT producto.id_producto, producto.cantidad FROM producto WHERE id_producto = %s", (data_insert["valor_id"],))
...
def test_registrar_salida_exito_disminuye_stock(create_jwt):
    data_insert = {"2valor_id": 1, "cantidad_salida": 10}

    response = create_jwt.post("/registrar_salida", data=data_insert)

    pool,cursor = instancia_conexion.iniciar_conexion()
    producto = instancia_conexion.uno(cursor, "SELECT producto.id_producto, producto.cantidad FROM producto WHERE id_producto = %s", (data_insert["2valor_id"],))
    instancia_conexion.cerrar_conexion(cursor, pool)
...
def test_salida_mayor_a_existencia(create_jwt):
    data_insert = {"2valor_id": 2, "cantidad_salida": 100}
...
def test_registrar_ajuste_negativo_stock(create_jwt):
    data_insert = {"4valor_id": 3, "cantidad_ajuste": 1, "tipo": "1"}

    response = create_jwt.post("/registrar_Ajuste", data=data_insert)

    pool,cursor = instancia_conexion.iniciar_conexion()
    producto = instancia_conexion.uno(cursor, "SELECT producto.id_producto, producto.cantidad FROM producto WHERE id_producto = %s", (data_insert["4valor_id"],))
    instancia_conexion.cerrar_conexion(cursor, pool)
...
def test_registrar_ajuste_negativo_stock_mayor_existencia(create_jwt):
    data_insert = {"4valor_id": 3, "cantidad_ajuste": 40, "tipo": "1"}
...
def test_registrar_ajuste_positivo_stock(create_jwt):
    data_insert = {"4valor_id": 3, "cantidad_ajuste": 24, "tipo": "2"}
...
def test_registrar_devolucion_cliente(create_jwt):
    data_insert = {"3valor_id": 3, "motivo": "test devolucion de cliente", "cantidad_devolucion": 5, "tipo": "2"}

    response = create_jwt.post("/registrar_devolucion", data=data_insert)

    pool,cursor = instancia_conexion.iniciar_conexion()
    producto = instancia_conexion.uno(cursor, "SELECT producto.id_producto, producto.cantidad FROM producto WHERE id_producto = %s", (data_insert["3valor_id"],))
    instancia_conexion.cerrar_conexion(cursor, pool)
...
def test_registrar_devolucion_proveedor(create_jwt):
    data_insert = {"3valor_id": 3, "motivo": "test devolucion a proveedor", "cantidad_devolucion": 5, "tipo": "1"}
...
def test_registrar_devolucion_proveedor_excede_cantidad(create_jwt):
    data_insert = {"3valor_id": 3, "motivo": "test devolucion a proveedor excede", "cantidad_devolucion": 50, "tipo": "1"}
